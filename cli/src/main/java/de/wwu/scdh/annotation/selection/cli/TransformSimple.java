...
@Command(
		...
		description =
				"transforms a *s*imple pair of XPath selector and RFC5147 character scheme selector along with a XSL transformation")
public class TransformSimple extends AbstractNormalize implements Callable<Integer> {
	...
	@Parameters(paramLabel = "RESOURCE", description = "The file (preimage) the selector selects from")
	URI resource;
	...
	public Integer call() throws Exception {
		...
		Point point;
		if (backward
				&& RFC5147CharScheme.class.isAssignableFrom(preimage.getImage().getPointClass())) {
		...
		if (backward) {
			...
			rewriter = factory.getRewriter(
					preimage.getImage().getPointClass(), XPathRefinedByRFC5147CharScheme.class, getRewriterConfig());
		} else {
			factory = new ForwardMappingFactory(compiler);
			rewriter = factory.getRewriter(
					XPathRefinedByRFC5147CharScheme.class, preimage.getImage().getPointClass(), getRewriterConfig());
		}
