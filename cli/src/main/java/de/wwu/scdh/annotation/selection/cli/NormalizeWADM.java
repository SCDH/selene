...
@Command(name = "normalize", mixinStandardHelpOptions = true, description = "normalize Web/Open Annotations selectors")
public class NormalizeWADM extends AbstractNormalize implements Callable<Integer> {

	@Parameters(paramLabel = "ANNOTATIONS", description = "The annotations given as URL or file path")
	URI selectors;
	...
	@Option(
			...
			paramLabel = "SOURCE",
			description =
					"The file all the ANNOTATIONS' selectors select from. This can be used to override oa:hasSource.")
	URI resource = null;
	...
	public Integer call() throws Exception {
		...
		try {
			model = NormalizeAnnotation.normalize(
					dom, resource, getNormalizerFactory(), getRewriterConfig(), selectorsResolved.toString(), lang);
		} catch (Exception e) {
			System.err.println(e.getMessage());
			return 10;
		}
