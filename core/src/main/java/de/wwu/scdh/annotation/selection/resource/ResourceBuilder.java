...
public class ResourceBuilder {
	...
	public static MappedDOMResource mapWithXsltTracePackage(
			DOMResource preimage,
			URI stylesheet,
			InputStream traceStream,
			String traceSystemId,
			Class<? extends Point> imagePointClass)
	 ...
	 */
	public static MappedDOMResource mapWithXsltTracePackage(
			DOMResource preimage, URI stylesheet, URI tracePkg, Class<? extends Point> imagePointClass)
			throws ResourceException {
		...
		try {
			...
			return ResourceBuilder.mapWithXsltTracePackage(
					preimage, stylesheet, traceStream, tracePkg.toString(), imagePointClass);
		} catch (MalformedURLException e) {
			throw new ResourceException(e.getMessage());
		} catch (IOException e) {
			throw new ResourceException(e.getMessage());
	 ...
	 */
	public static Class<? extends Point> pointerClassFromOutputMethod(Xslt30Transformer transformer)
			throws ResourceException {
	 ...
	 */
	public static Processor processorFromModifiedConfig(Source config) throws ResourceException {
		try {
			...
			log.error(
					"failed to modify Saxon config file {} with {}",
					config.getSystemId(),
					ResourceBuilder.class.getResource(MODIFY_CONFIG_XSL));
			throw new ResourceException(e);
		} catch (URISyntaxException e) {
			log.error(
					"failed to load XSLT for modifying Saxon config {}",
					ResourceBuilder.class.getResource(MODIFY_CONFIG_XSL));
			throw new ResourceException(e);
			// } catch (IOException e) {
