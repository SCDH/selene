...
public class TestResourceBuilder {
	...
	public static final URI LIBTRACE_XML = Paths.get("src", "main", "resources", "xslt", "libtrace-xml.xsl")
			.toFile()
			.toURI();
	...
	public static final URI ID_XSL =
			Paths.get("src", "test", "resources", "xsl", "id.xsl").toFile().toURI();
	...
	public void testGesangParsedWithXerces() throws ResourceException {
		...
		while (tree.hasNext()) {
			...
			// log(node.getNodeKind().toString() + " " + node.getNodeName() + " " +
			// String.valueOf(MappedDOMResource.getNodeTrace(node, MappedDOMResource.ID_XPATH)));
			assertTrue(MappedDOMResource.getNodeTrace(node, MappedDOMResource.ID_XPATH)
					.isPresent());
			assertTrue(preimage.idToPreimageNode.containsKey(
					MappedDOMResource.getNodeTrace(node, MappedDOMResource.ID_XPATH)
							.get()));
			assertEquals(
					node,
					preimage.idToPreimageNode.get(MappedDOMResource.getNodeTrace(node, MappedDOMResource.ID_XPATH)
							.get()));
		}
	}
		...
		while (items.hasNext()) {
			...
			if (XdmNode.class.isAssignableFrom(item.getClass())) {
				...
				while (treeIterator.hasNext()) {
					...
					log("id: " + node.getNodeKind().toString() + " " + node.getNodeName() + " "
							+ String.valueOf(MappedDOMResource.getNodeTrace(node, MappedDOMResource.TRACKING_XPATH)));
					if (node.getNodeKind().equals(XdmNodeKind.DOCUMENT)) continue;
					assertTrue(MappedDOMResource.getNodeTrace(node, MappedDOMResource.TRACKING_XPATH)
							.isPresent());
				}
			}
		}
	...
	public void testGesangForwardMapping() throws ResourceException, SaxonApiException {
		...
		while (tree.hasNext()) {
			XdmNode node = tree.next();
			log("forward: " + node.getNodeKind().toString() + " "
					+ String.valueOf(MappedDOMResource.getNodeTrace(node, MappedDOMResource.ID_XPATH)));
			assertTrue(MappedDOMResource.getNodeTrace(node, MappedDOMResource.ID_XPATH)
					.isPresent());
			...
			assertEquals(
					1,
					preimage.getCorrespondingInImage(node).size(),
					"preimage node: " + node.getNodeName().toString());
		}
	}
		...
		while (seq.hasNext()) {
			...
			if (item.isNode()) {
				...
				while (tree.hasNext()) {
					...
					assertNotEquals(
							null,
							preimage.getCorrespondingInPreimage(node),
							"image node: " + node.getNodeName()); // fails: is null
					assertTrue(
							preimage.getCorrespondingInPreimage(node).isPresent(), "image node: " + node.getNodeName());
					if (!node.getNodeName().equals("trace:text")) {
						// assertEquals(node.getNodeName(),
						// preimage.getCorrespondingInPreimage(node).get().getNodeName());
					}
